 ...
 const [parent] = useAutoAnimate()
  return (
    <div ref={parent} className='w-60 mr-3 h-[29rem] min-w-60 max-md:hidden rounded-lg flex flex-col gap-1 sepet'>
      ...
      <div ref={parent}>
      {basket.map(item => (<div ref={parent} key={item.id}  className='border w-[90%] mx-auto    h-6 rounded-lg whitespace-nowrap overflow-visible urun-text  mb-2 flex font-fontb  justify-evenly '>
        <div className='urun-text' >{item.name}</div>-
        <div className='urun-text' >{item.price} TL </div>
        <div onClick={() => dispatch(deleteBasket(item.id))}  className='text-xs cursor-pointer text-ararenk3' >{item.count} sil</div>
        </div>)) }
      </div>
